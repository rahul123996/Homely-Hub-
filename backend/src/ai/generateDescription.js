...
const listAmenities = (amenities) => {
  if (!amenities || !Array.isArray(amenities) || amenities.length === 0) return "Not provided";
  return amenities.map((item) => (typeof item === "object" && item !== null ? item.name || item.title || item : item)).join(", ");
};

const readAddress = (address) => {
  if (!address) return "Not provided";
  if (typeof address === "string") return address;
  return [address.area, address.city, address.state, address.pinCode || address.pincode]
    .filter(Boolean)
    .join(", ");
};

const generateDescription = async (property) => {
  const propertyInfo = `- Property Name: ${property.propertyName || property.name || "Not provided"}
- Extra Information: ${property.extraInfo || "Not provided"}
- Property Type: ${property.propertyType || "Not provided"}
- Room Type: ${property.roomType || "Not provided"}
- Maximum Guests: ${property.maximumGuest || "Not provided"}
- Amenities: ${listAmenities(property.amenities)}
- Price per Night: ${property.price ? `Rs ${property.price}` : "Not provided"}
